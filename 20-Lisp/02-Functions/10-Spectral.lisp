...
(defmacro rm-type (lst type)
  `(remove-if #',type ,lst))

;;(rm-type '(1 2 3 4 5 6 7 8) evenp)

(defun fm-mod (carrier modulator index)
  (loop for i from 1 to index
    ; the higher the index, the greater is the amount of sidebands.
    ; harm is the overtone from which  the modulation is calculated.
    for pos = (+ carrier (* i modulator))
    for neg = (abs (- carrier (* i modulator)))
    ; sidebands variables
    when (not (zerop pos)) collect pos into reslis
    ; when not zero collect the upper sideband 
    when  (not (zerop neg)) collect neg into reslis
    ; collect the lower sideband and convert it to midi
    ; only when not zero and carrier > modulator
...
(defun fm-mod-ratio (frequency c-ratio m-ratio index)
  (let* ((divisor (apply #'gcd (list c-ratio m-ratio)))
         (carrier (* frequency (/ c-ratio divisor)))
         (modulator (* frequency (/ m-ratio divisor))))
    (print (list (/ c-ratio divisor)  (/ m-ratio divisor)))
    (loop for i from 1 to index
      ; the higher the index, the greater is the amount of sidebands.
      ; harm is the overtone from which the modulation is calculated.
      for pos = (+ carrier (* i modulator))
